...
public:
	...
	static FString ResolveUniqueName(const FString& BaseName, const FString& UUID, bool bHasPriority); // Used in Media Framework
	static TSharedPtr<FOWLSpoutSender, ESPMode::ThreadSafe> GetOrCreateSender(
		const FString& ResolvedName, 
		const FString& UUID, 
		bool bFixGamma,
		bool bHasPriority);
	
	static TSharedPtr<FOWLSpoutSender, ESPMode::ThreadSafe> GetSender(const FString& UUID);
	static void ReleaseSender(const FString& UUID);
