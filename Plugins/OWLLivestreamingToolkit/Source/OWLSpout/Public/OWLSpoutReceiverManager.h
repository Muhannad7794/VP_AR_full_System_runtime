...
struct OWLSPOUT_API FOWLSpoutReceiverInterface
{
	...
	FOWLSpoutReceiverInterface()
	: Name(FOWLSpoutNames::UseFirstAvailableSenderName)
	{
	}
...
public:
	void Receive()
	{
		...
		SpoutReceiver->ReceiveRenderTarget(Name, RenderTarget, bFixGamma);
	}
...
public:
	...
	/* Sets the receiver of index `Index` to active. Returns true if sender found, false if not */
	UFUNCTION(BlueprintCallable, Category = "Off World Live Spout Receiver Settings")
	bool ActivateReceiver(int Index);

	/* Sets the receiver of index `Index` to inactive. Returns true if sender found, false if not */
	UFUNCTION(BlueprintCallable, Category = "Off World Live Spout Receiver Settings")
