...
UCLASS(meta=(DisplayName="OWL Spout Sender Manager"))
...
public:
	...
	/* Creates a new spout sender interface and adds it to the `SpoutSenders` array */
	UFUNCTION(BlueprintCallable, Category = "OWL Spout Sender Manager")
	FOWLSpoutSenderInterface AddNewSender(FString Name, UTextureRenderTarget2D* TextureTarget,
										FString StandaloneName = "", bool bActive = true, bool bFixGamma = true);

	/* Utility function to create a new spout sender - returns the struct created which has been added to the SpoutSenders array */
	UFUNCTION(BlueprintCallable, Category = "OWL Spout Sender Manager")
	FOWLSpoutSenderInterface CreateSender(FString Name, UTextureRenderTarget2D* TextureTarget,
										FString StandaloneName = "", bool bActive = true, bool bFixGamma = true);
	...
	/* Activates a sender by the array index. Returns true if found. */
	UFUNCTION(BlueprintCallable, Category = "OWL Spout Sender Manager")
	bool ActivateSenderByIndex(int Index);

	/* Deactivates a sender by the array index. Returns true if found. */
	UFUNCTION(BlueprintCallable, Category = "OWL Spout Sender Manager")
	bool DeactivateSenderByIndex(int Index);
