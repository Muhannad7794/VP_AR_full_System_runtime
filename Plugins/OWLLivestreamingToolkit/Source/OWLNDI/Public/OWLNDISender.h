...
public:
	...
	// Recovery mechanism
	bool bWaitingForValidResolution = false;
	bool AttemptResolutionRecovery(UTextureRenderTarget2D* SrcTarget, ENDIVideoConversionFormat VideoConversionFormat);
	uint32 RecoveryAttemptCount = 0;
	const uint32 RecoveryAttemptInterval = 30; // Only check every 30 frames
...
private:
	...
	TSharedPtr<ISubmixBufferListener, ESPMode::ThreadSafe> SharedThis;
