...
public:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Output Settings|Advanced")
    int AudioBufferSize = 100;
    ...
    /* Starts the media output. Warning, in some situations such
     as when this is called on boot, this can be async. Use `OnStart` delegate
     to ensure completion. */
    UFUNCTION(BlueprintCallable, Category = "OWL Media Output Component")
    bool Start();

    /* Stops the media output component. This is blocking */
    UFUNCTION(BlueprintCallable, Category = "OWL Media Output Component")
    void Stop();
