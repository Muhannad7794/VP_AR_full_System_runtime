	...
	if (Client && bIsRunning)
	{
		if (frameData.bIsValid)
		{
			...
			else
			{
				...
				// Bypass the Engine's Virtual Subject check. 
				// Rely ONLY on the plugin's internal array to ensure Static Data is always sent on the first frame.
				if (!Subjects.Contains(SubjectName))
				{
					...
					if (Client->GetSources().Num() > 0)
					{
						...
						// Force Static Data to initialize the Subject and turn it Green
...
FString FZEDLiveLinkSource::GetConnectionString() const
{
	// This bridges the gap: It grabs Stereolabs' perfectly formatted string
	// and physically hands it to Unreal Engine's Preset Saver.
	return ZEDSettings.ToString();
}
